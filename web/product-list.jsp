		...
		<div class="last-view">
			...
			<dl class="clearfix">
				<c:forEach var="recent"items="${recent}">
					<dt><img src="product."></dt>
					<dd><a href="product.do?id=${product.epId}&action=detail" target="_blank">${product.epName}</a></dd>
				</c:forEach>
			</dl>
		...
		<div class="product-list">
			...
			<div class="pager">
				<ul class="clearfix">
					...
					<c:if test="${pageIndex>1}"><li><a href="/product.do?action=list&pageIndex=${pageIndex-1}">上一页</a></li>
					</c:if>
					<c:forEach var="page" begin="1" end="${totalPage}">
						<li><c:if test="${pageIndex==page}"> class="current"</li>></li>
					</c:forEach>
					<c:if test="${pageIndex<totalPage}"><li><a href="/product.do?action=list&pageIndex=${pageIndex+1}">下一页</a></li>
					</c:if>
				</ul>
			</div>
			<div class="clear"></div>
			<ul class="product clearfix">
				...
				<c:forEach var="pro" items="${productList}">
					<li>
						<dl>
							<dt><a href="/product.do?id=${pro.epId}&action=detail" target="_blank"><img src="${pro.epFileName}" /></a></dt>
